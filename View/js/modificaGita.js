window.onload = async () => {
    ...
    xhr.open("GET", `../index.php/mostraGitina?id='${idEvento}'`, true);
...
function visualizzaGita(data) {
    ...
    for (i = 0; i < data.length; i++) {
        ...
        stringa += `<div class="utente">
                <form action="../index.php/modificaGita?id='${data[i].id}'" method="POST">
                    <div class="sopra">
                        <span>${data[i].id} -</span>
                        <input type="text" name="nome" value="${data[i].nome}">
                        <input type="text" name="descrizione" value="${data[i].descrizione}">
                        <input type="mail" name="data" value="${data[i].data}">
